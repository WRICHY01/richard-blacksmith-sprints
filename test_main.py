...
TEST_EMAIL = "eoihd@gmai.com"

def _wipe_test_rows():
    """Deletes any rows for TEST_EMAIL, regardless of how they got there."""
    supabase.table("user_sessions").delete().eq("user_email", TEST_EMAIL).execute()
    supabase.table("users").delete().eq("email", TEST_EMAIL).execute()


@pytest.fixture(scope="session", autouse=True)
def wipe_before_session():
    """
    Runs ONCE before any test in the run starts. This is what makes the
    suite self-healing: it doesn't matter whether the last run crashed,
    was Ctrl+C'd, or failed its own teardown — this wipe doesn't depend
    on anything that happened last time, so stale rows can never survive
    into a new run.
    """
    _wipe_test_rows()
...
def run_around_tests():
    """Cleans up TEST_EMAIL's rows after every test, pass or fail."""
    yield
    _wipe_test_rows()
...
def test_no_session_token_matching_from_db_due_to_token_mismatch():
...
def test_user_signout_no_active_session():
    """
    Verifies that the /signout endpoint prevent user with no active session
    """
